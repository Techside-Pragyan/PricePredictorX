...
async def predict_stock(ticker: str, days: int = 7):
    ...
    model, metrics, test_preds, test_actual = model_service.train_lstm_model(df_preprocessed)
    
    # Prepare last sequence for future prediction
    scaled_data = model_service.scaler.transform(df_preprocessed[['Close']].values)
    last_sequence = scaled_data[-60:]
    
    future_preds = model_service.predict_future(model, last_sequence, days, model_service.scaler)
    
    # Flatten metrics and predictions for JSON
    return {
        "ticker": ticker,
        "metrics": metrics,
